...
class HttpClient {
	...
	private async handleResponse(response: Response) {
		if (!response.ok) {
			throw new ApiError(`Api error! Status: ${response.status}`, response.status);
		}
	...
	private async handleNetworkError(response: Response) {
		if (!response.ok) {
			throw new NetworkError(`Network error on request. Status: ${response.status}`);
		}
	}

	private async request<T>(url: string, method: string, data?: object): Promise<T> {
